...
// Var
const weakmap = new WeakMap(); // directoryインスタンスをkeyに、対になるdirectory情報Mapが入る。
		...
		if( cache.has(pathStr) ){
			...
			return super(pathStr, stats).then( ({_this, stats, fullpath})=>{
				if( !stats.isDirectory() ){
					throw new Error(`Not directory path: ${fullpath}`);
				}
				// 対になる内容物一覧のMap
				const map = new Map();
				weakmap.set(_this, map);
				return _this;
			});
		}
	}
...
/*
	weakmapから、自身と対になるmapインスタンスを取得する
		その際に前回の更新情報が未セットなら読み込む
	引数
		Directoryインスタンス
	返り値
		promise
			ディレクトリ構成情報のmapを引数に解決する。
*/
async function getMap(dir){
	...
	const map = weakmap.get(dir);
	if( not.number(map.date) ){
