...
// Var
const {cache, isStats, symbol_path} = require('./shared.js');
		...
		自身の実体の名前を変更する
			...
			返り値
				promise
					自身を引数に解決する。
	*/
	async rename(name, overwrite=false){
		console.log(`Base#rename()`, `before: ${this.name}`, `after: ${name}`, `overwrite: ${overwrite}`);
		// Validation
		if( not.str(name) ){
			throw new TypeError(`Invalid arguments[0]: ${name}`);
		}
		if( not.bool(overwrite) ){
			throw new TypeError(`Invalid argument[1]: ${overwrite}`);
		}

		const path_new = path.join(this.base, name);
		// 上書き対応でやっつけ振り分け
		if( overwrite ){
			await fse.move(this.path, path_new, {
				overwrite: true
			});
		}else{
		...
		this.name = name;
		this[symbol_path] = path_new; // パス更新
		return this;
	}
...
// Alias
Base.prototype.getParentDir = Base.prototype.getParentDirectory;
